...
class PodcastEditor:
    ...
    def setup_menu(self):
        ...
        file_menu.add_command(label="Speichern als...", command=self.save_project_as, accelerator="Ctrl+Shift+S")
        file_menu.add_separator()
        file_menu.add_command(label="Exportieren...", command=self.export_audio, accelerator="Ctrl+E")
        file_menu.add_separator()
        ...
        edit_menu.add_command(label="Zeile einfügen", command=self.insert_line, accelerator="Ctrl+Enter")
        edit_menu.add_command(label="Zeile löschen", command=self.delete_line, accelerator="Ctrl+D")
        edit_menu.add_separator()
        edit_menu.add_command(label="Sprecher hinzufügen...", command=self.add_speaker, accelerator="Ctrl+Shift+S")
    ...
    def setup_left_panel(self, parent):
        ...
        ttk.Button(btn_frame, text="➕ Hinzufügen", command=self.add_speaker).pack(side=tk.LEFT, padx=2)
        ttk.Button(btn_frame, text="✏️ Bearbeiten", command=self.edit_speaker).pack(side=tk.LEFT, padx=2)
        ttk.Button(btn_frame, text="🗑️ Entfernen", command=self.remove_speaker).pack(side=tk.LEFT, padx=2)
        ...
        lang_combo = ttk.Combobox(filter_frame, textvariable=self.lang_var, values=["de", "en"], width=8)
        ...
        ttk.Button(voice_frame, text="Als Sprecher verwenden", command=self.use_voice_as_speaker).pack(pady=5)
    ...
    def setup_syntax_tags(self):
        ...
        for i, color in enumerate(colors, 1):
            self.script_editor.tag_config(f"speaker{i}", foreground=color, font=('Consolas', 11, 'bold'))
        
        # Emotion-Tag
        self.script_editor.tag_config("emotion", foreground=self.colors['emotion'], font=('Consolas', 10, 'italic'))
        
        # Pause-Tag
        self.script_editor.tag_config("pause", foreground=self.colors['pause'])
        
        # Kommentar-Tag
        self.script_editor.tag_config("comment", foreground='#6a9955', font=('Consolas', 10, 'italic'))
...
"""
    ...
    def show_voice_library(self):
        ...
        messagebox.showinfo("Voice Library", f"Verfügbare Stimmen: {self.voice_library.get_voice_count()}")
...
class SpeakerDialog:
    ...
    def __init__(self, parent, voice_library, speaker: Optional[Speaker] = None):
        ...
        ttk.Button(voice_frame, text="📂", command=self.browse_voice, width=3).pack(side=tk.LEFT, padx=2)
        ttk.Button(voice_frame, text="🎨", command=self.select_from_library, width=3).pack(side=tk.LEFT)
    ...
    def browse_voice(self):
        ...
        filepath = filedialog.askopenfilename(
            title="Voice-Sample auswählen",
            filetypes=[("Audio", "*.wav *.mp3"), ("Alle", "*.*")]
        )
        if filepath:
