...
class MultiTrackEditor(ttk.Frame):
    ...
    def _create_toolbar(self):
        ...
        # Add Track
        ttk.Button(toolbar, text="➕ Voice Track",
                   command=lambda: self.add_track("Voice Track", TrackType.VOICE)
                   ).pack(side=tk.LEFT, padx=2)
        
        ttk.Button(toolbar, text="🎵 Music Track",
                   command=lambda: self.add_track("Music Track", TrackType.MUSIC)
                   ).pack(side=tk.LEFT, padx=2)
        
        ttk.Button(toolbar, text="🔊 SFX Track",
                   command=lambda: self.add_track("SFX Track", TrackType.SFX)
                   ).pack(side=tk.LEFT, padx=2)
        
        ttk.Separator(toolbar, orient=tk.VERTICAL).pack(side=tk.LEFT, padx=5, fill=tk.Y)
        
        # Import Clip
        ttk.Button(toolbar, text="📁 Import Clip",
                   command=self.import_clip).pack(side=tk.LEFT, padx=2)
        
        # Export
        ttk.Button(toolbar, text="💾 Export Audio",
                   command=self.export_audio).pack(side=tk.LEFT, padx=2)
    ...
    def _create_mixer_panel(self, parent) -> ttk.Frame:
        ...
        ttk.Label(mixer_frame, text="MIXER", font=("Arial", 12, "bold")
                  ).pack(side=tk.TOP, pady=5)
        
        # Scrollable mixer
        mixer_canvas = tk.Canvas(mixer_frame, width=self.MIXER_WIDTH)
        mixer_scrollbar = ttk.Scrollbar(mixer_frame, orient=tk.VERTICAL,
                                        command=mixer_canvas.yview)
    ...
    def _add_mixer_strip(self, track: Track):
        ...
        mute_btn = ttk.Button(
            control_frame, text="M",
            width=3,
            command=lambda: self._toggle_mute(track.id)
        )
        ...
        solo_btn = ttk.Button(
            ...
            width=3,
            command=lambda: self._toggle_solo(track.id)
        )
    ...
    def _render_track(self, track: Track, y: int):
        ...
        self.timeline_canvas.create_rectangle(
            0, y, canvas_width, y + self.TRACK_HEIGHT,
            fill="#2d2d2d",
            outline="#3d3d3d",
            tags=("track", track.id)
        )
