...
class TimelineEditor(ttk.Frame):
    ...
    def _setup_ui(self):
        ...
        ttk.Button(toolbar, text="⏮️", width=3, 
                   command=self._goto_start).pack(side=tk.LEFT, padx=1)
        ttk.Button(toolbar, text="⏪", width=3,
                   command=self._skip_backward).pack(side=tk.LEFT, padx=1)
        ttk.Button(toolbar, text="▶️", width=3,
                   command=self._play).pack(side=tk.LEFT, padx=1)
        ttk.Button(toolbar, text="⏸️", width=3,
                   command=self._pause).pack(side=tk.LEFT, padx=1)
        ttk.Button(toolbar, text="⏩", width=3,
                   command=self._skip_forward).pack(side=tk.LEFT, padx=1)
        ttk.Button(toolbar, text="⏭️", width=3,
                   command=self._goto_end).pack(side=tk.LEFT, padx=1)
        ...
        ttk.Checkbutton(toolbar, text="Snap", variable=self.snap_var,
                        command=self._toggle_snap).pack(side=tk.LEFT, padx=10)
        
        # Snap Interval
        ttk.Label(toolbar, text="Grid:").pack(side=tk.LEFT, padx=5)
        self.snap_combo = ttk.Combobox(toolbar, width=8,
                                       values=["0.1s", "0.5s", "1.0s"],
                                       state="readonly")
    ...
    def add_scene(self, 
                  scene: Scene,
                  position: Optional[float] = None) -> Scene:
        """
        Füge Szene hinzu
