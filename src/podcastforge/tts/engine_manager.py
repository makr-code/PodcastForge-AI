    ...
    def synthesize(self, 
        ...
        try:
            audio = self.model.synthesize(
                text,
                speaker_id=int(speaker) if speaker.isdigit() else 0
            )
...
class TTSEngineManager:
    ...
    def get_engine(self, 
                   ...
                   auto_load: bool = True) -> BaseTTSEngine:
        """
        ...
        try:
            ...
            self.engine_usage[engine_key] = 1
            logger.info(f"Engine loaded: {engine_key} ({engine.get_memory_usage() / 1024**3:.2f} GB)")
            return engine
    ...
    def get_stats(self) -> Dict:
        ...
        return {
            ...
            'total_memory': sum(
                e.get_memory_usage() 
                for e in self.loaded_engines.values()
            ) / 1024**3  # GB
        }
    
    def __repr__(self) -> str:
        return (f"TTSEngineManager("
