...
class VoiceLibrary:
    ...
    def search(self,
               ...
               tags: Optional[List[str]] = None) -> List[VoiceProfile]:
    ...
    def suggest_for_podcast_style(self,
        ...
        if style == PodcastStyle.INTERVIEW:
            ...
            if host and guest:
                suggestions = [host[0], guest[0] if guest else host[1] if len(host) > 1 else host[0]]
        
        elif style == PodcastStyle.DOCUMENTARY:
            # Dokumentarfilm-Stimme
            doc_voices = self.search(
                language=language,
                style=VoiceStyle.DOCUMENTARY
            )
            if doc_voices:
